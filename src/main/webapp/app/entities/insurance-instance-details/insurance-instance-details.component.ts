...
export class InsuranceInstanceDetailsComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.insuranceInstanceDetailsService
      ...
      .subscribe(
        (res: HttpResponse<IInsuranceInstanceDetails[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInInsuranceInstanceDetails(): void {
    this.eventSubscriber = this.eventManager.subscribe('insuranceInstanceDetailsListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IInsuranceInstanceDetails[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/insurance-instance-details'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
    ...
    this.insuranceInstanceDetails = data || [];
  }
